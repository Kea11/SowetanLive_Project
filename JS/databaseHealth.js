const hel = document.querySelector('.para');
const Healthlist = document.querySelector('.HealthComment');
const formForHealth = document.querySelector('.formForHealth');
...
const addHComment = (Comments, id) => {
    ...
    Healthlist.innerHTML += htmlComment;
  };

db.collection('Blogs').get().then((snapshot) => {
    Health(snapshot.docs[1].data());
}).catch(err =>{
console.log(err);
});
...
formForHealth.addEventListener('submit', e =>{
        e.preventDefault();
    
        const now = new Date();
        const comment = {
    
       comment:formForHealth.userComment.value,
        created_at:firebase.firestore.Timestamp.fromDate(now),
        location:formForHealth.userLocation.value
